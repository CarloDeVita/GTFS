...
BEGIN 
    ...
    CREATE TEMPORARY TABLE TEMPORARY_PASSAGES
    (
        segment INTEGER,
        vehicle TEXT,
    ...
    LOOP
        IF(trip_id IS NULL OR trip_id <> cur_rec.trip) THEN -- new trip being calculated
            ...
            IF(trip_id IS NOT NULL) THEN
                ...
                SELECT segment, vehicle, passage_time
                FROM TEMPORARY_PASSAGES;

                TRUNCATE TEMPORARY_PASSAGES;
            END IF;
            ...
            LOOP
                ...
                IF(active[current_day+1] = TRUE) THEN -- if trip is active on that day
                    INSERT INTO TEMPORARY_PASSAGES(SEGMENT,VEHICLE,PASSAGE_TIME) 
